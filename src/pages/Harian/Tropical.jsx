...
function Tropical() {
  ...
  useEffect(() => {
    const fetchGaleri = async () => {
      try {
        const response = await fetch('http://localhost:3333/api/galeris/kategori/Harian/subCategory/Tropical');
  ...
  return (
    <div className="font-sans text-gray-900 min-h-screen overflow-auto">
      ...
      <div className="flex flex-col items-center justify-center h-full bg-pink-50 mt-2">
        <h2 className="text-5xl font-bold text-center mt-16 mb-8">Tropical</h2>
        ...
        <p className="text-black text-justify max-w-5xl px-6 mb-8">
          Kelebihan tropical adalah memberikan tampilan yang santai dan terbuka, menciptakan kesan sehat dan segar. Selain itu, gaya ini pilihan sempurna untuk tetap tampil modis dan nyaman di cuaca panas. Dengan bahan dan motif yang sesuai, gaya ini mencerminkan semangat santai namun tetap elegan.
        </p>
      </div>
      ...
      {selectedImage && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white p-4 w-full max-w-sm max-h-[90vh] h-auto overflow-auto relative">
            ...
            <img src={`http://localhost:3333${selectedImage}`} alt="Selected Tropical" className="w-full mb-4 max-w-full" />
