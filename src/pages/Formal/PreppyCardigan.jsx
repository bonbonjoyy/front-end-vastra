...
import { faEye } from '@fortawesome/free-solid-svg-icons'; // Eye icon for viewing image
...
function PreppyCardigan() {
  ...
  useEffect(() => {
    ...
    const fetchGaleri = async () => {
      try {
        const response = await fetch('http://localhost:3333/api/galeris/kategori/Formal/subCategory/PreppyCardigan');
      ...
      {selectedImage && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white p-4 w-full max-w-sm max-h-[90vh] h-auto overflow-auto relative">
            ...
            <img src={`http://localhost:3333${selectedImage}`} alt="Selected Preppy Cardigan" className="w-full mb-4 max-w-full" />
